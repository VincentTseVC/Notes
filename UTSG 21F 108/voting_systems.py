...
def clean_data(data: List[List[str]]) -> None:
    ...
    """
    for row in data:
        row[COLUMN_RIDING] = int(row[0])

        row[COLUMN_VOTER] = int(row[1])

        rank_ballots = row[COLUMN_RANK].split(';')
        for i in range(len(rank_ballots)):
            rank_ballots[i] = rank_ballots[i].upper()
        row[COLUMN_RANK] = rank_ballots

        range_ballots = row[COLUMN_RANGE].split(';')
        for i in range(len(range_ballots)):
            range_ballots[i] = int(range_ballots[i])
        row[COLUMN_RANGE] = range_ballots

        approval_ballots = row[COLUMN_APPROVAL].split(';')
        for i in range(len(approval_ballots)):
            approval_ballots[i] = approval_ballots[i] == APPROVAL_TRUE
        row[COLUMN_APPROVAL] = approval_ballots
...
def voting_approval(av_ballots: List[List[bool]]) -> List[int]:
    ...
    """
    res = [0, 0, 0, 0]
    for av_ballot in av_ballots:
        for i in range(len(av_ballot)):
            if av_ballot[i]:
                res[i] += 1
    return res
...
def voting_range(range_ballots: List[List[int]]) -> List[int]:
    ...
    """
    res = [0, 0, 0, 0]
    for range_ballot in range_ballots:
        for i in range(len(range_ballot)):
            res[i] += range_ballot[i]
    return res
...
def voting_borda(rank_ballots: List[List[str]]) -> List[int]:
    ...
    for rank_ballot in rank_ballots:
        res[PARTY_ORDER.index(rank_ballot[0])] += 3
        res[PARTY_ORDER.index(rank_ballot[1])] += 2
        res[PARTY_ORDER.index(rank_ballot[2])] += 1
    return res
...
['CPC', 'GREEN', 'LIBERAL']]
    """
    for rank_ballot in rank_ballots:
        rank_ballot.remove(party_to_remove)
...
def get_lowest_not_removed(votes: List[int], removed_parties: List[str]) -> str:
    ...
    for i in range(len(PARTY_ORDER)):
        if PARTY_ORDER[i] not in removed_parties:
            avaliable_parties.append(PARTY_ORDER[i])
            avaliable_votes.append(votes[i])

    return avaliable_parties[avaliable_votes.index(min(avaliable_votes))]

    a = votes[:]
    for itm in removed_parties:
        a[PARTY_ORDER.index(itm)] += sum(a)
    return PARTY_ORDER[a.index(min(a))]
...
def voting_irv(rank_ballots: List[List[str]]) -> str:
    ...
    while True:
        votes = voting_plurality(extract_column(rank_ballots, 0))
        
        # only considered the first choice
        first_choice = max(votes)
        # if majority
        if max(votes) > sum(votes)/2:
            return PARTY_ORDER[votes.index(first_choice)]
        else:
            # remove the lowest
...
def points_to_winner(points_per_party: List[int]) -> str:
    ...
    'GREEN'
    """
    return PARTY_ORDER[points_per_party.index(max(points_per_party))]
