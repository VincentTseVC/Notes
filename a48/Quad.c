...
Quad *BST_insert(Quad *root, Quad *new_node) {
  ...
  if (new_node->key == root->key)
    ...
    root->left = BST_insert(root->left, new_node);  // D
  else
    root->right = BST_insert(root->right, new_node); // A B
  return root;
}
...
Quad *BST_search(Quad *root, int tx, int ty) {
  ...
  if (root->tx == tx && root->ty == ty) return root;
  else if ((tx + (ty * root->sx)) < root->key)
...
Quad *BST_delete(Quad *root, int tx, int ty) {
  ...
  if (key == root->key) {
    ...
    // have both children
      ...
      // copy paste the content within the node
      root->tx = temp->tx;
      root->ty = temp->ty;
      root->w = temp->w;
      root->h = temp->h;
...
int get_colour(Image *im, Quad *q) {
  ...
  for (int w = 0; w < q->w; w++)
    for (int h = 0; h < q->h; h++)
      colour += im->data[(q->tx + w) + ((q->ty + h) * q->sx)];
...
int similar(Image *im, Quad *q, int threshold) {
  ...
  return (maxCol - minCol) <= threshold;
}
...
Quad *split_tree_helper(Image *im, Quad *root, int threshold, Quad *new_root) { 
  if (root != NULL) {
    ...
    // if all the pixel within the quad are similar, just add it to the new BST
    if (similar(im, root, threshold))
      new_root = BST_insert(new_root, new_Quad(root->tx, root->ty, root->w, root->h, root->wsplit, root->sx));
    // otherwise, we need to to some spliting
    else
    {
      if (root->wsplit) {   // == 1
        quad1 = new_Quad(root->tx, root->ty, root->w / 2, root->h, 0, root->sx);
        quad2 = new_Quad(root->tx + root->w / 2, root->ty, root->w / 2, root->h, 0, root->sx);
      }
      else
      {
        quad1 = new_Quad(root->tx, root->ty, root->w, root->h / 2, 1, root->sx);
        quad2 = new_Quad(root->tx, root->ty + root->h / 2, root->w, root->h / 2, 1, root->sx);
      }


    // quad1 = new_Quad(root->tx, 
    //                  root->ty, 
    //                  root->wsplit ? root->w / 2 : root->w, 
    //                  root->wsplit ? root->h : root->h / 2, 
    //                  root->wsplit ? 0 : 1, root->sx);


      new_root = BST_insert(new_root, quad1);
      new_root = BST_insert(new_root, quad2);
    }
...
Quad *split_tree(Image *im, Quad *root, int threshold) {
  ...
  Quad *new_root = split_tree_helper(im, root, threshold, NULL);
  delete_BST(root);
  return new_root;
}
...
void drawOutline(Image *im, Quad *root, unsigned char col) {
  ...
  if (root == NULL) return;
  // x + (y * sx)
  for (int w = 0; w < root->w; w++) {
    im->data[(root->tx + w) + ((root->ty) * root->sx)] = col; // first row
    im->data[(root->tx + w) + ((root->ty + root->h - 1) * root->sx)] = col; // last row
  }

  for (int h = 0; h < root->h; h++) {
    im->data[(root->tx) + ((root->ty + h) * root->sx)] = col; // left most column
    im->data[(root->tx + root->w - 1) + ((root->ty + h) * root->sx)] = col; // right most column
  }
  ...
  // for (int w = 0; w < root->w; w++)
  //   for (int h = 0; h < root->h; h++)
  //     if (w == 0 || h == 0 || w == root->w - 1 || h == root->h - 1)
  //       im->data[(root->tx + w) + ((root->ty + h) * root->sx)] = col;
...
void save_Quad(Image *im, Quad *root) {
  ...
  unsigned char col = get_colour(im, root);
