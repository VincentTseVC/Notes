"""Prep 9 Synthesize: Expression Trees
...
class BoolOp(Expr):
    ...
    each separated by the SAME operator:
    ...
    def evaluate(self) -> Any:
        ...
        for expr in self.values:
            if self.op == 'and':
                res = res and expr.evaluate()
            elif self.op == 'or':
                res = res or expr.evaluate()
            else:
                raise ValueError(f'Invalid operator {self.op}')
        return res
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        left = self.left
        for comp, expr in self.comparisons:
            if comp == "<=":
                if left.evaluate() > expr.evaluate():
                    return False
                left = expr
        ...
        # for comp, expr in self.comparisons:
        #     if comp == '<':
        #         values.append(Bool(left.evaluate() < expr.evaluate()))
        #         left = expr
        #     elif comp == '<=':
        #         values.append(Bool(left.evaluate() <= expr.evaluate()))
        #         left = expr
        #     else:
        #         raise ValueError(f'Invalid operator {comp}')
        #
        # return BoolOp('and', values).evaluate()
