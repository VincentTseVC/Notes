...
def _test_mixed(comment_file_path, student_dir):
  ...
  sent = "mixed echo test $mysh"
  expected = "mixed echo test $mysh"
  execute_echo_test(comment_file_path, sent, expected)
...
def _test_mixed_error(comment_file_path, student_dir, command_wait=0.01):
  ...
  try:
    ...
    if "ERROR: input line too long" not in output2:
      finish(comment_file_path, "NOT OK")  
      return 
