...
class DriverRequest(Event):
    ...
    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        ...
        if passenger is not None:
            ...
            events.append(Pickup(self.timestamp + travel_time, passenger, self.driver))
        return events
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {}: Request a passenger".format(self.timestamp, self.driver)
...
class Cancellation(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {}: Cancel Request".format(self.timestamp, self.passenger)
...
class Pickup(Event):
    ...
    def __init__(self, timestamp: int, passenger: Passenger, driver: Driver) -> None:
        """Initialize a PassengerRequest event.
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {} pick up {}".format(self.timestamp, self.driver, self.passenger)

    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:

        self.driver.end_drive()

        monitor.notify(self.timestamp, PASSENGER, PICKUP, self.passenger.id, self.passenger.origin)
        ...
        if self.passenger.status == WAITING:
            ...
            events.append(Dropoff(self.timestamp + travel_time, self.passenger, self.driver))
...
class Dropoff(Event):
    ...
    def __str__(self) -> str:
        ...
        """
        return "{} -- {} dropoff {}".format(self.timestamp, self.driver, self.passenger)

    def do(self, dispatcher: Dispatcher, monitor: Monitor) -> List[Event]:
        self.driver.end_trip()

        monitor.notify(self.timestamp, PASSENGER, DROPOFF, self.passenger.id, self.passenger.destination)
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            if event_type == "DriverRequest":
                ...
                rider = Passenger(id, patience, origin, destination)
                event = PassengerRequest(timestamp, rider)
            events.append(event)
