...
class Monitor:
    ...
    def _average_total_distance(self) -> float:
        ...
        num_driver = 0
        total_dist = 0
        for activites in self._activities[DRIVER].values():

            for i in range(len(activites) - 1):
                total_dist += manhattan_distance(activites[i].location, 
                                                    activites[i+1].location);
            num_driver += 1
    
        if num_driver == 0: return 0.0
        return total_dist / num_driver
    ...
    def _average_trip_distance(self) -> float:
        ...
        for activites in self._activities[DRIVER].values():
            ...
            if len(activites) > 2:
                total_dist += manhattan_distance(activites[1].location,
                                                 activites[2].location)
            num_driver += 1
