...
def read_course_data(file: str) -> dict:
    ...
    return {course: transform_course_data(data[course]) for course in data}
...
def transform_course_data(course_data: dict) -> tuple[str, str, set]:
    ...
    sections = {transform_section_data(sdata) for sdata in course_data['sections']}
...
def transform_section_data(section_data: dict) -> tuple[str, str, tuple]:
    ...
    Preconditions:
        - section_data is a dictionary containing data about a single section, in the format
          found in course_data_small.json.
    """
    code = section_data['sectionCode']
    term = section_data['term']
    meetings = tuple(transform_meeting_time_data(mdata) for mdata in section_data['meetingTimes'])
    return code, term, meetings
...
def transform_meeting_time_data(meeting_time_data: dict) \
    ...
    """
    day = meeting_time_data['day']
    start = datetime.time(int(meeting_time_data['startTime'].split(':')[0]))
    end = datetime.time(int(meeting_time_data['endTime'].split(':')[0]))
...
def get_valid_schedules(course_data: dict[str, tuple[str, str, set]],
    ...
    """
    c1, c2, c3, c4, c5 = tuple(filter_by_term(course_data[c], term) for c in courses)
...
def filter_by_term(course: tuple[str, str, set], term: str) -> tuple[str, str, set]:
    ...
    Preconditions:
      - term in {'F', 'S'}
    """
    return course[0], course[1], {s for s in course[2] if s[1] in term or s[1] == 'Y'}
...
if __name__ == '__main__':
    ...
    data = read_course_data('data/course_data_small.json')


    import pprint

    pprint.pprint(data['CSC110'])
