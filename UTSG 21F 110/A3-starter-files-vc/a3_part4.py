...
def load_data(filename: str) -> tuple[list[WeatherMetrics], list[FfwiOutput]]:
    ...
    with open(filename) as f:
        ...
        for row in reader:
            ...
            inputs_so_far.append(WeatherMetrics(int(row[0]),
                                                int(row[1]),
                                                float(row[2]),
                                                float(row[3]),
                                                float(row[4]),
            ...
            inputs_so_far.append(FfwiOutput(float(row[6]),
                                            float(row[7]),
                                            float(row[8]),
                                            float(row[9]),
                                            float(row[10]),
...
def calculate_ffwi_outputs(readings: list[WeatherMetrics]) -> dict[tuple[int, int], FfwiOutput]:
    ...
    for wm in readings:
        ffmc = ffwi.calculate_ffmc(wm, ffmc)
        dmc = ffwi.calculate_dmc(wm, dmc)
        dc = ffwi.calculate_dc(wm, dc)

        isi = ffwi.calculate_isi(wm, ffmc)
        bui = ffwi.calculate_bui(dmc, dc)
        fwi = ffwi.calculate_fwi(isi, bui)

        res[(wm.month, wm.day)] = FfwiOutput(ffmc, dmc, dc, isi, bui, fwi)
...
def get_xy_data(outputs: dict[tuple[int, int], FfwiOutput], attribute: str) -> \
    ...
    for t in outputs:
        month, day = t
        output = outputs[t]
        keys.append(str(month) + ", " + str(day))
        values.append(getattr(output, attribute))
